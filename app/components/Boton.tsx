...
const Boton = ({ url, theme, desc, favicon }: Props) => {
  ...
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className={`flex items-center gap-2 cursor-pointer group relative bg-(--color-buttonBg) rounded-lg border-2 border-black px-3  py-1.5 text-sm font-semibold text-black transition-all hover:-translate-x-1 hover:-translate-y-1 hover:shadow-[4px_4px_0_rgb(0,0,0)] hover:after:absolute hover:after:-bottom-2 hover:after:-right-2 hover:after:left-0 hover:after:top-0 hover:after:-z-10 theme-${theme}`}>
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src={faviconUrl} alt={`${dominio} favicon`} className="w-5 h-5 rounded-full transition-all duration-300 overflow-hidden flex-none" />
